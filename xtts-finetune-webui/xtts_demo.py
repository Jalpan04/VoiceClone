            ...
            def preprocess_dataset(audio_path, audio_folder_path, language, whisper_model, out_path, train_csv, eval_csv, progress=gr.Progress(track_tqdm=True)):
                ...
                if not audio_files:
                    ...
                    try:
                        ...
                        # Skip Whisper if it's not needed (Offline Mode)
                        try:
                            asr_model = WhisperModel(whisper_model, device=device, compute_type=compute_type, local_files_only=True)
                        except:
                            print(" > Offline Mode: Skipping Whisper download.")
                            asr_model = None
