...
def train_gpt(custom_model,version, language, num_epochs, batch_size, grad_acumm, train_csv, eval_csv, output_path, max_audio_length=255995):
    ...
    if not os.path.isfile(DVAE_CHECKPOINT) or not os.path.isfile(MEL_NORM_FILE):
        ...
        print(" > Offline Mode: DVAE files missing! Please ensure they are in base_models folder.")
    ...
    if not os.path.isfile(TOKENIZER_FILE) or not os.path.isfile(XTTS_CHECKPOINT):
        ...
        # )
        print(f" > Offline Mode: XTTS v{version} files missing! Please ensure they are in base_models folder.")
...
# Use from ready folder
    ...
    # On Windows, multiprocessing data workers cause file lock errors. Force 0.
    num_workers = 0
    ...
    # The coqui formatter does not set the 'language' key that XTTSDataset
    # needs for key_samples_by_col(). Inject it manually here.
    for s in train_samples:
        if "language" not in s or not s["language"]:
            s["language"] = language
    for s in eval_samples:
        if "language" not in s or not s["language"]:
            s["language"] = language
    print(f" > Injected language='{language}' into {len(train_samples)} train and {len(eval_samples)} eval samples")
