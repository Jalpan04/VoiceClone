...
class XTTSDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        # We use a loop instead of recursion to avoid RecursionError on Windows/Linux
        for retry_count in range(100):
            if self.is_eval:
                # In eval mode, we try the requested index first, then sequential ones if it fails
                current_index = (index + retry_count) % len(self.samples)
                sample = self.samples[current_index]
                sample_id = str(current_index)
            else:
                if retry_count == 0:
                    # First try the requested index if possible (though trainer usually passes random)
                    # but since self.samples is a dict in train mode, we just go straight to random
                    pass
                
                # select a random language
                lang = random.choice(list(self.samples.keys()))
                # select random sample
                current_index = random.randint(0, len(self.samples[lang]) - 1)
                sample = self.samples[lang][current_index]
                # a unique id for each sample to deal with fails
                sample_id = lang + "_" + str(current_index)

            # ignore samples that we already know are not valid
            ...
            try:
                ...
                if self.debug_failures:
                    logger.warning("Error loading %s: %s", sample.get("audio_file", "unknown"), str(e))
                self.failed_samples.add(sample_id)
                continue

            # check if the audio and text size limits
            if (
                wav is None
                or (self.max_wav_len is not None and wav.shape[-1] > self.max_wav_len)
                or (self.max_text_len is not None and tseq.shape[0] > self.max_text_len)
                ...
                if self.debug_failures:
                     logger.warning("Sample %s out of bounds, skipping.", sample.get("audio_file", "unknown"))
                self.failed_samples.add(sample_id)
                continue
            ...
            res = {
                ...
                "cond_lens": (
                    torch.tensor(cond_len, dtype=torch.long) if not torch.is_tensor(cond_len) and not (isinstance(cond_len, float) and torch.isnan(torch.tensor(cond_len))) else torch.tensor([cond_len])
                ),
                "cond_idxs": torch.tensor(cond_idxs) if not torch.is_tensor(cond_idxs) and not (isinstance(cond_idxs, float) and torch.isnan(torch.tensor(cond_idxs))) else torch.tensor([cond_idxs]),
            }
            return res
        
        raise RuntimeError(f"XTTSDataset: Failed to find a valid sample after 100 retries. Check your dataset and vocab.json!")
